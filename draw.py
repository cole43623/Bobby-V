...
def update_player(self, key):
    ...
    if key == 1: # LEFT
        if x_idx > 0 and self.layout[y_idx][x_idx - 1] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            flag = True
            self.player_pos[0] -= 32
    elif key == 2: # RIGHT
        if x_idx < self.lencol - 1 and self.layout[y_idx][x_idx + 1] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            flag = True
            self.player_pos[0] += 32
    elif key == 3: # UP
        if y_idx > 0 and self.layout[y_idx - 1][x_idx] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            self.player_pos[1] -= 32
            flag = True
    elif key == 4: # DOWN
        if y_idx < self.lenrow - 1 and self.layout[y_idx + 1][x_idx] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            self.player_pos[1] += 32
            flag = True
